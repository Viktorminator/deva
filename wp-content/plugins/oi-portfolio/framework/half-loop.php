...
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
    ...
    <?php $thumb_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '') ?>
	 	<?php
        	if (get_post_meta($post->ID, 'oi_th', 1) == 'portfolio-squre'){ $col='col-md-6 oi_x1'; $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full');};
			if (get_post_meta($post->ID, 'oi_th', 1) == 'portfolio-squrex2'){ $col='col-md-6 oi_x1'; $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full');};
			if (get_post_meta($post->ID, 'oi_th', 1) == 'portfolio-wide'){ $col='col-md-6 oi_x1'; $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full');};
			if (get_post_meta($post->ID, 'oi_th', 1) == 'portfolio-long'){ $col='col-md-6 oi_x1'; $large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full');};
		?>
